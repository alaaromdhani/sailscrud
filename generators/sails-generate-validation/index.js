  ...
  before(scope, done) {
    function DataTypeToJsConverter(type){
      ...
      if(dataType =='ENUM'){
        ...
        return 'array().items('+JSON.stringify(values)+')'
      }
      if(dataType ==  'JSON'){
        return 'object()'
      }
    ...
    /*primaryKey:true,
      autoIncrement:true*/
    function getAttributeValidationFunction(model,attribute){
        let validationFunction ="joi"
        let options = model.attributes[attribute]
        if(!options.autoIncrement){
          if(DataTypeToJsConverter(options.type)!=''){
            validationFunction+='.'+DataTypeToJsConverter(options.type)
            
          }
          if((options.allowNull==false || options.required || options.primaryKey ) && !options.defaultValue){
    ...
    function convertToJoiObject(model){
        let joiObject = {}
       
        Object.keys(model.attributes).forEach(attribute=>{
          if(getAttributeValidationFunction(model,attribute)!='joi'){
            joiObject[attribute] = getAttributeValidationFunction(model,attribute)
          }
    ...
    if (_.isUndefined(scope.args[0])) {
      return done('Please provide a name for this validation.'.yellow);
    }
    ...
    return  fs.readdir('./api/models',(err,files)=>{
      if(err){
        console.log(err)
      }
      else{
        ...
        scope.filename = `${scope.entityName}Schema.js`;
    
        scope.entity = _.snakeCase(scope.args[0]);
        scope.joiObject = {}

        const modelFile = files.filter(i=>i.toLocaleLowerCase()===scope.entity.toLocaleLowerCase()+'.js')
         
        if(modelFile.length>0){
          
          const modelObject = require('../../api/models/'+modelFile[0].replace('.js',''))
          scope.joiObject = convertToJoiObject(modelObject)
        }
        else{
          console.log(scope.entity+'does not correspond to entity so all the configuration will be ignored')
        }
        console.log('Generating a new validation for entity as ', scope.entity);
        this.targets['./utils/validations/:filename'] = {}
        this.targets['./utils/validations/:filename'].template = 'validation.template';
        done()
      }

    })  
    /*if(sails.models[scope.entity]){
        console.log(convertToJoiObject(sails.models[scope.entity]))
