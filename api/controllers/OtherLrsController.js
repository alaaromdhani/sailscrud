...
module.exports = {
    getActivityState:async (req,res)=>{
        ...
        let activity = await OtherInteractive.findOne({where:{id:activityId}})
            if(dbAgent&&activity){
                    let activityState = await ActivityState.findOne({where:{agent_id:dbAgent.id,other_interactive_id:activity.id,deprecated:false}})
                     if(!activityState){
                        const data = 'ActivityState('+activityId+'|'+agent+': resume'
                        try{
                            const actState = await ActivityState.create({
                                other_interactive_id:activity.id,
                                agent_id:dbAgent.id,
        ...
        if(dbAgent && activity){
            try{
                ...
                const activityState = await ActivityState.findOne({
                    where:{
                        other_interactive_id:activityId,
                        agent_id:dbAgent.id,
                        deprecated:false                        
                    }
                })
    ...
    putStatement:async (req,res)=>{
       ...
       if(timestamp && statementId && agentName && verbId && verbName && objectId){
        ...
        let activityState = await ActivityState.findOne({where:{agent_id:agent.id,other_interactive_id:courseId,deprecated:false}})
            if(!activityState){
            ...
            if(!obj){
                ...
                [custom_object,created] = await CustomObject.findOrCreate({
                    ...
                    defaults:{
                        ...
                        other_interactive_id:object.id.split('/')[0],
                        agent_id:agent.id
                    }
                })
                ...
                try{
                    ...
                    if(custom_object){
                        ...
                        await Statement.create({
                            ...
                            jsonData:req.body,
                            obj_id:obj.id,
                            other_interactive_id: obj.other_interactive_id,
                            verb_id:verbId,
                            timeStamp:timestamp,
                            agent_id:agent.id
